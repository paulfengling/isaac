...
class ReachingUR10eTask(VecTask):
    def __init__(self, cfg):
        ...
        # ur10e 6 dof + 2 gripper
        self.cfg["env"]["numObservations"] = 17  # 8 dof pos + 8 dof vel + 1 progress
        ...
        self._end_effector_link = "panda_leftfinger"  # ur10e末端执行器link名，需与urdf一致
        ...
        if self._control_space == "cartesian":
            ...
            # ur10e 末端执行器的索引需与urdf一致
            self.jacobian_end_effector = self.jacobian[:, self.rigid_body_dict_robot[self._end_effector_link], :, :6]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        robot_asset_file = "urdf/ur10e_panda/robots/ur10e_panda.urdf"  # 修改为你的urdf路径
        ...
        # ur10e 6 dof + 2 gripper
        robot_dof_stiffness = torch.tensor([400, 400, 400, 400, 400, 400, 1.0e6, 1.0e6], dtype=torch.float32, device=self.device)
        robot_dof_damping = torch.tensor([80, 80, 80, 80, 80, 80, 1.0e2, 1.0e2], dtype=torch.float, device=self.device)
    ...
    def init_data(self):
        # ur10e 6关节+2夹爪，默认初始角度可根据实际urdf调整
        self.robot_default_dof_pos = torch.tensor([0, -1.57, 1.57, 0, 1.57, 0, 0, 0], device=self.device, dtype=torch.float32)
    ...
    def compute_observations(self):
        ...
        robot_dof_pos = self.dof_pos[:, :8]
        robot_dof_vel = self.dof_vel[:, :8]
        ...
        generalization_noise = torch.rand((dof_vel_scaled.shape[0], 6), device=self.device) + 0.5
        ...
        self.obs_buf[:, 13:16] = target_pos
        self.obs_buf[:, 16] = dof_pos_scaled[:, 6].mean()  # 可选：夹爪状态
    ...
    def pre_physics_step(self, actions):
        ...
        if self._control_space == "joint":
            targets = self.robot_dof_targets[:, :6] + self.robot_dof_speed_scales[:6] * self.dt * actions[:, :6] * self._action_scale
            # 夹爪动作直接赋值
            if actions.shape[1] > 6:
                targets = torch.cat([targets, actions[:, 6:8]], dim=1)
            else:
                targets = torch.cat([targets, self.robot_dof_targets[:, 6:8]], dim=1)
        ...
        self.robot_dof_targets[:, :8] = torch.clamp(targets, self.robot_dof_lower_limits, self.robot_dof_upper_limits)
